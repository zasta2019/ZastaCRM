...
{
    private $contact_data = array('firstname', 'lastname', 'email', 'phonenumber', 'title', 'password', 'send_set_password_email', 'donotsendwelcomeemail', 'permissions', 'direction','invoice_emails','estimate_emails','contract_emails','task_emails','project_emails');
            ...
            foreach ($permissions as $permission) {
                ...
                if($not_manual_request == true) {
                    if($permission == 6){
                        ...
                        $this->db->where('id',$contact_id);
                        $this->db->update('tblcontacts',array('invoice_emails'=>1));
                    }
                }
            }
    ...
    public function get_statement($customer_id, $from, $to)
    {
        ...
        $result['beginning_balance'] = $this->db->query('
            SELECT (SUM(tblinvoices.total) - (SELECT COALESCE(SUM(tblinvoicepaymentrecords.amount),0)
            FROM tblinvoicepaymentrecords
            JOIN tblinvoices ON tblinvoices.id = tblinvoicepaymentrecords.invoiceid
